...
- `src/db.ts` — the persistence adapter for durable conversations. Flue's record, not the business record.
- `src/store/` — the audit store: the evidence, open questions and run history that outlive a single run.
- `src/cli/` — the human side of an audit: founding one, and answering the questions a run escalated.

## Commands

- `npm run audit -- new --id <auditId> --objective "<text>"` — found an audit. A run needs one.
- `npx flue run src/agents/fieldguide.ts --new --id <runId> --data '{"auditId":"<auditId>"}' --message "<objective>"` — one run against it.
- `npm run audit -- show --id <auditId>` — what the audit holds; `-- retention` prints what is kept and why.
...
- `docs/diary.md` — add an entry at each milestone worth telling a follower about: a capability that now works, a direction that changed, a notable success or failure. Newest first. Plain language for a non-technical reader: no code, no jargon, no ADR numbers. Tell the story of working through the plan; avoid time-based references (`this week`, `today`, `recently`), which read oddly for someone arriving later. The technical record stays in `decisions.md` and git history.
